...
def tokenize(expr: str) -> list:
    ...
    if expr_not_recognize != expr:
        ...
        if wrong_char.search(expr):
            raise CalcError(f"Ошибка: Неизвестный токен.")
...
def shunting_yard(expr: str) -> float|int:
    ...
    for data in output:
        try:
            ...
            if data == "~":
                op1 = stack.pop()
                stack.append(operations[data](op1))
            elif data in operations:
                if len(stack) < 2: raise CalcError(f"Ошибка: Недостаточно операндов.")
                op2 = stack.pop()
                op1 = stack.pop()
                ...
                stack.append(operations[data](op1, op2))
